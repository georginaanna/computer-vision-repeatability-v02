...
for(i in 1:length(trainf)){
  ...
  nrowtemp <- min(max(minbox,  
                  nrow(temp) - round(rnorm(n = 1, mean = comp_best_mean, sd = comp_sd), 0)), nrow(temp))
  
  temp2 <- sample(x = 1:nrow(temp), size = nrowtemp)

  write.table(x = temp[temp2, ], 
              paste(compbestdir, traindir, trainf[i], '.txt', sep=''),
              sep = ' ',
              row.names = F,
              col.names = F)
...
for(i in 1:length(valf)){
  ...
  write.table(x = temp[temp2, ], 
              paste(compbestdir, valdir, valf[i], '.txt', sep=''),
              sep = ' ',
              row.names = F,
              col.names = F)
